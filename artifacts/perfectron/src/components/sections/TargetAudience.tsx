    ...
    <section className="bg-gray-50 py-24">
      <div className="container mx-auto max-w-6xl px-4">
        <div className="text-center mb-16">
          <h2 className="text-3xl font-bold text-gray-900 md:text-4xl">이런 분들께 추천드립니다</h2>
          <p className="mt-4 text-lg text-gray-600">어떤 상황에서도 퍼펙트론이 해결책을 찾아드립니다</p>
        </div>
          ...
          {targets.map((target, index) => (
            <Card key={index} className="border-t-4 border-t-primary shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="pt-6">
                <div className={`mb-4 inline-flex rounded-xl p-3 ${target.color}`}>
                  <target.icon className="h-6 w-6" />
                </div>
                <h3 className="mb-2 text-xl font-bold text-gray-900">{target.title}</h3>
                <p className="mb-4 text-sm font-medium text-gray-500">{target.question}</p>
